...
int method_com_friends_info_request(struct server *sv, struct client *ct, json_t *json)
{
	json_error_t json_err;
	json_t *rsp_json = json_object();
	struct raw_packet *packet = malloc_raw_packet(sv, ct);

	json_object_set_new(rsp_json, "method", json_string("com.friends.info.respond"));
